...
type Payload={profile?:{goal?:string;calories?:number;protein?:number;carbs?:number;fat?:number};today?:{calories?:number;protein?:number;carbs?:number;fat?:number;water?:number;mealCount?:number};whoop?:{connected?:boolean;recovery?:number|null;sleep?:number|null;strain?:number|null;hrv?:number|null}};

function fallback(p:Payload){
  const profile=p.profile||{};const today=p.today||{};const whoop=p.whoop||{};
...
export async function POST(request:NextRequest){
  ...
  if(!apiKey)return NextResponse.json(fallback(payload));
  ...
  try{
    const r=await fetch("https://api.openai.com/v1/responses",{method:"POST",headers:{Authorization:`Bearer ${apiKey}`,"Content-Type":"application/json"},body:JSON.stringify({model:process.env.NOURA_AI_MODEL||"gpt-5.6-luna",input:prompt,max_output_tokens:500}),cache:"no-store"});
